...
short_description: Manage PyPI proxy repositories
"""
...
def main():
    endpoint_path_to_use = "/pypi/proxy"
    argument_spec = NexusHelper.nexus_argument_spec()
    argument_spec.update(
        format=dict(type="str", choices=["pypi"], required=False),
        type=dict(type="str", choices=["proxy"], required=False),
        pypi=dict(
            type='dict',
            apply_defaults=True,
            options=dict(
                remove_quarantined=dict(type="bool", default=True),
            ),
        ),
    )
    ...
    if module.params["state"] == "present":
        ...
        additional_data = {
            "pypi": NexusHelper.camalize_param(helper, "pypi"),
        }
