...
def update_routing_rule(helper):
    ...
    if (state == "present"):
        ...
        if(rule_exists):
            no_update_needed = all(existing_data[k] == v for k, v in data.items() if k in existing_data)
...
def main():
    ...
    argument_spec.update(
        ...
        mode=dict(type='str', required=False, no_log=False, default='block', choices=['allow', 'block' ]),
        matchers=dict(type='list', elements='str', required=False, no_log=False, default=list()),
