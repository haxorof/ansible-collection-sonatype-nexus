...
def get_capability(helper, match_type):
    ...
    existing_capability = next((item for item in content_list if item["type"] == match_type), None)
...
def main():
    ...
    content = {}
    changed = True
    match_type = module.params["type"]
    capability_exists, existing_capability, capability_id = get_capability(helper, match_type)

    if module.params["state"] == "present":
        if capability_exists is True:
            content, changed = update_capability(helper, existing_capability, capability_id)
        else:
