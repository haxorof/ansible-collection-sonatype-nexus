...
def main():
    ...
    argument_spec.update(
        format=dict(type="str", choices=["nuget"], required=False),
        type=dict(type="str", choices=["proxy"], required=False),
        nuget_proxy=dict(
            type='dict',
            apply_defaults=True,
            options=dict(
                query_cache_item_max_age=dict(type="int", default=3600),
                nuget_version=dict(type="str", choices=["V2", "V3"], default="V3"),
            ),
        ),
    )
